...
def line(frame: int, a: int, b: int):
    return [(
        float(df.loc[frame, :, :, a].X),
        float(df.loc[frame, :, :, a].Y),
        float(df.loc[frame, :, :, a].Z)),
        (float(df.loc[frame, :, :, b].X),
         float(df.loc[frame, :, :, b].Y),
         float(df.loc[frame, :, :, b].Z))]
...
def update(frame):
    ...
    ax.scatter(df.loc[frame, :, :, :].X, df.loc[frame,
               :, :, :].Y, df.loc[frame, :, :, :].Z)


df = pd.read_csv('test.csv').set_index(['Frame', 'Time', 'Hand', 'Part'])

joint_comb = pd.read_csv('mcapt/hand_joint_conb.csv',
